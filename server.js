...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/") return html(res, pageV2());
...
async function processUpload(req, res, url) {
  ...
  const job = {
    ...
    createdAt: now,
    updatedAt: now,
    config: { originalName, serverUrl, appId, cardName, purchaseUrl, jumpText, jumpUrl, obfuscate, useVmp, fullVmp },
...
async function buildApk(job) {
  const { id } = job;
  const { originalName, serverUrl, appId, cardName, purchaseUrl, jumpText, jumpUrl, obfuscate, useVmp, fullVmp } = job.config;
  ...
  updateJob(job, "正在获取 AES-GCM/RSA-3072 安全协议公钥");
  const transport = await resolveV2Transport(serverUrl, appId);
  ...
  const transportMessage = "安全传输：RSA-3072 密钥封装 + AES-256-GCM + HMAC-SHA256 + Android Keystore ECDSA 设备绑定 + 时间戳/随机数防重放；客户端不再内置共享密钥";
  const vmpMessage = fullVmp
    ? "已启用整包 VMP 兼容模式：全部应用 DEX 函数代码抽取、运行时回填、随机壳包、原生壳段加密、垃圾类、签名证书自校验、反调试及 Frida/Xposed/LSPosed 运行时监测"
    : useVmp
    ? `已启用内置 VMP+：配置字符串加密、随机指令验证虚拟机、反调试及 Frida/Xposed/LSPosed 监测、${dexHashes.length} 个原始 DEX 完整性校验`
    : "未启用 VMP+ 保护";
  ...
  return {
    ...
    jumpUrl,
    transportMessage,
    signingMessage: `APK 已使用 ${signingCertificate.algorithm}-${signingCertificate.keyBits} 发布证书签名，证书 SHA-256：${signingCertSha256}`,
...
async function ensureSigningKey(tools) {
  ...
  if (configured) {
    ...
    if (!storepass || !keypass) throw new Error("已配置 APK 签名库，但缺少签名密码 Secret");
    const keystore = path.join(TOOLS, "runtime-release-signing.p12");
    let decoded;
    try {
      decoded = Buffer.from(configured.replace(/\s+/g, ""), "base64");
    } catch (_) {
      throw new Error("APK 签名库 Base64 无效");
    }
    if (decoded.length < 1024) throw new Error("APK 签名库内容无效");
  ...
  if (fs.existsSync(keystore) && fs.existsSync(metadataFile)) {
    ...
    if (!metadata.storepass || !metadata.keypass || !metadata.alias) throw new Error("本地发布签名配置损坏");
...
async function resolveV2Transport(serverUrl, expectedAppId) {
  const endpoint = `${normalizeUrl(serverUrl)}/api/v2/config?build=${Date.now()}`;
  let response;
  try {
    ...
    throw new Error(`无法读取强加密协议公钥：${error.message || error}`);
  }
  if (!response.ok) throw new Error(`强加密协议未启用：HTTP ${response.status}`);
  const config = await response.json();
  if (Number(config.v) !== 2 || config.appId !== expectedAppId || !config.keyId || !config.publicKey) {
    throw new Error("服务器 v2 安全协议配置无效或 App ID 不匹配");
  }
  const publicDer = Buffer.from(String(config.publicKey).replace(/-/g, "+").replace(/_/g, "/"), "base64");
  let publicKey;
  try {
    publicKey = crypto.createPublicKey({ key: publicDer, format: "der", type: "spki" });
  } catch (_) {
    throw new Error("服务器 RSA 公钥格式无效");
  }
  const bits = publicKey.asymmetricKeyDetails && publicKey.asymmetricKeyDetails.modulusLength;
  if (publicKey.asymmetricKeyType !== "rsa" || !bits || bits < 3072) {
    throw new Error("服务器传输密钥强度不足，至少需要 RSA-3072");
  }
  return { keyId: String(config.keyId), publicKey: String(config.publicKey), modulusLength: bits };
}
...
function writeJavaSources(root, packageName, launcher, serverUrl, appId, keyId, serverPublicKey, signingCertSha256, cardName, purchaseUrl, jumpText, jumpUrl, useVmp, dexHashes) {
...
${configString("SERVER_RSA_PUBLIC_KEY", serverPublicKey)}
...
import android.content.*; import android.content.pm.*; import android.os.*; import android.util.Base64; import java.io.*; import java.security.*; import java.util.*; import java.util.zip.*;
final class VmpRuntime {
  ...
  static boolean check(Context context){
    ...
    if(integrityState==0){ synchronized(VmpRuntime.class){ if(integrityState==0)integrityState=(verifySignature(context)&&verifyDex(context))?1:-1; } }
    if(integrityState==1)startMonitor();
    return integrityState==1;
  }
  private static boolean compromised(){ return Debug.isDebuggerConnected()||Debug.waitingForDebugger()||traced()||hooked(); }
  ...
  private static boolean hooked(){
    ...
    String[] files={"/proc/self/maps","/proc/net/unix"}; String[] marks={"frida","gum-js-loop","xposed","substrate","lsposed","edxp","riru","zygisk"};
    for(String file:files){try{BufferedReader r=new BufferedReader(new FileReader(file));String line;while((line=r.readLine())!=null){String lower=line.toLowerCase(Locale.US);for(String mark:marks)if(lower.contains(mark)){r.close();return true;}}r.close();}catch(Throwable ignored){}}
    return false;
  }
  private static void startMonitor(){ if(monitorStarted)return; synchronized(VmpRuntime.class){if(monitorStarted)return;monitorStarted=true;Thread t=new Thread(new Runnable(){public void run(){while(true){try{Thread.sleep(2500);}catch(InterruptedException ignored){}if(compromised()){integrityState=-1;android.os.Process.killProcess(android.os.Process.myPid());return;}}}},"runtime-integrity");t.setDaemon(true);t.start();} }
...
final class LicenseClient {
  ...
  LicenseResult activate(String cardKey,String deviceId,String appVersion)throws Exception{ JSONObject p=new JSONObject().put("cardKey",cardKey).put("cardName",LicenseConfig.CARD_NAME).put("deviceId",deviceId).put("appVersion",appVersion); return request("/api/v2/activate",p); }
  LicenseResult heartbeat(String cardKey,String deviceId,String appVersion)throws Exception{ JSONObject p=new JSONObject().put("cardKey",cardKey).put("cardName",LicenseConfig.CARD_NAME).put("deviceId",deviceId).put("appVersion",appVersion); return request("/api/v2/heartbeat",p); }
  private LicenseResult request(String path,JSONObject payload)throws Exception{ ${useVmp ? 'if(!VmpRuntime.check(context))throw new SecurityException("runtime integrity check failed"); ' : ''}Exception last=null; for(String base:LicenseConfig.getBaseUrls(context)){ try{ Envelope env=seal(path,payload); return once(base,path,env); }catch(Exception e){last=e;} } throw last==null?new IllegalStateException("network verify failed"):last; }
  private LicenseResult once(String base,String path,Envelope env)throws Exception{ HttpURLConnection c=(HttpURLConnection)new URL(base+path).openConnection(); c.setRequestMethod("POST"); c.setConnectTimeout(20000); c.setReadTimeout(20000); c.setDoOutput(true); c.setRequestProperty("Content-Type","application/json; charset=utf-8"); c.setRequestProperty("Cache-Control","no-store"); OutputStream o=c.getOutputStream(); o.write(env.body.toString().getBytes(StandardCharsets.UTF_8)); o.close(); int status=c.getResponseCode(); InputStream in=status>=400?c.getErrorStream():c.getInputStream(); String text=readAll(in); if(status>=400){ try{throw new IllegalStateException(new JSONObject(text).optString("message","server rejected request"));}catch(JSONException ignored){throw new IllegalStateException("server rejected request: "+status);} } JSONObject data=open(path,new JSONObject(text),env); boolean ok=data.optBoolean("ok",false); int code=data.optInt("code",-1); long expiresAt=data.optLong("expiresAt",0); ${useVmp ? 'ok=VmpRuntime.accept(ok,code,expiresAt); ' : ''}return new LicenseResult(ok,code,data.optString("message",""),expiresAt,data.optLong("remainingSeconds",0),data.optLong("nextHeartbeatSeconds",180)); }
  private Envelope seal(String path,JSONObject payload)throws Exception{ long ts=System.currentTimeMillis()/1000L; String nonce=randomB64(18); byte[] key=new byte[32],iv=new byte[12]; random.nextBytes(key); random.nextBytes(iv); payload.put("ts",ts); String aad=aad(path,ts,nonce,"request"); Cipher aes=Cipher.getInstance("AES/GCM/NoPadding"); aes.init(Cipher.ENCRYPT_MODE,new SecretKeySpec(key,"AES"),new GCMParameterSpec(128,iv)); aes.updateAAD(aad.getBytes(StandardCharsets.UTF_8)); String data=b64(aes.doFinal(payload.toString().getBytes(StandardCharsets.UTF_8))); PublicKey rsa=KeyFactory.getInstance("RSA").generatePublic(new X509EncodedKeySpec(unb64(LicenseConfig.SERVER_RSA_PUBLIC_KEY))); Cipher wrap=Cipher.getInstance("RSA/ECB/OAEPWithSHA-256AndMGF1Padding"); wrap.init(Cipher.ENCRYPT_MODE,rsa,new OAEPParameterSpec("SHA-256","MGF1",MGF1ParameterSpec.SHA256,PSource.PSpecified.DEFAULT)); String wrapped=b64(wrap.doFinal(key)),ivText=b64(iv),deviceKey=DeviceKey.publicKey(context); String authenticated=aad+"\\n"+wrapped+"\\n"+ivText+"\\n"+data+"\\n"+deviceKey; String mac=b64(hmac(macKey(key),authenticated)); String deviceSig=DeviceKey.sign(context,authenticated); JSONObject body=new JSONObject().put("v",2).put("appId",LicenseConfig.APP_ID).put("keyId",LicenseConfig.TRANSPORT_KEY_ID).put("ts",ts).put("nonce",nonce).put("key",wrapped).put("iv",ivText).put("data",data).put("mac",mac).put("deviceKey",deviceKey).put("deviceSig",deviceSig); return new Envelope(body,key); }
  private JSONObject open(String requestPath,JSONObject e,Envelope env)throws Exception{ if(e.optInt("v")!=2||!LicenseConfig.APP_ID.equals(e.optString("appId"))||!LicenseConfig.TRANSPORT_KEY_ID.equals(e.optString("keyId")))throw new GeneralSecurityException("server identity mismatch"); long ts=e.optLong("ts"); if(Math.abs(System.currentTimeMillis()/1000L-ts)>300)throw new GeneralSecurityException("server timestamp invalid"); String nonce=e.optString("nonce"),iv=e.optString("iv"),data=e.optString("data"),mac=e.optString("mac"); String aad=aad(requestPath+":response",ts,nonce,"response"); String authenticated=aad+"\\n"+iv+"\\n"+data; if(!MessageDigest.isEqual(unb64(mac),hmac(macKey(env.key),authenticated)))throw new GeneralSecurityException("server HMAC invalid"); Cipher aes=Cipher.getInstance("AES/GCM/NoPadding"); aes.init(Cipher.DECRYPT_MODE,new SecretKeySpec(env.key,"AES"),new GCMParameterSpec(128,unb64(iv))); aes.updateAAD(aad.getBytes(StandardCharsets.UTF_8)); JSONObject payload=new JSONObject(new String(aes.doFinal(unb64(data)),StandardCharsets.UTF_8)); if(payload.optLong("ts")!=ts)throw new GeneralSecurityException("response timestamp mismatch"); return payload; }
  private String aad(String path,long ts,String nonce,String direction){ return "2\\n"+LicenseConfig.APP_ID+"\\n"+LicenseConfig.TRANSPORT_KEY_ID+"\\n"+path+"\\n"+ts+"\\n"+nonce+"\\n"+direction; }
  private static byte[] macKey(byte[] key)throws Exception{ MessageDigest d=MessageDigest.getInstance("SHA-256"); d.update(key); d.update("android-license-v2-hmac".getBytes(StandardCharsets.UTF_8)); return d.digest(); }
...
<body><main class="wrap">
  <h1>APK 验证与保护</h1>
  <div class="muted">上传已编译 APK，自动加入卡密验证并执行兼容型整包保护。</div>
  <div class="protocol"><span>RSA-3072</span><span>AES-256-GCM</span><span>HMAC-SHA256</span><span>Keystore ECDSA</span><span>时间戳防重放</span></div>

  <section class="panel">
    <h2>选择 APK</h2>
    <div id="drop" class="drop"><b id="fileName">点击选择或拖入 APK</b><div class="muted" style="margin-top:6px">文件只发送到当前 APK 处理服务</div><input id="file" type="file" accept=".apk" hidden></div>
  </section>

  <section class="panel">
    <h2>验证配置</h2>
    <div class="grid">
      <label class="wide">验证后台地址<input id="server" value="${DEFAULT_SERVER}"></label>
      <label>App ID<input id="appId" value="demo_android_app"></label>
      <label>卡密名称<input id="cardName" value="默认软件"></label>
      <label>卡密购买地址<input id="purchaseUrl" placeholder="不填则不显示"></label>
      <label>跳转文字<input id="jumpText" placeholder="例如：官方网站"></label>
      <label class="wide">跳转网址<input id="jumpUrl" placeholder="不填则不显示"></label>
    </div>
    <div class="checks">
      <label><input id="obfuscate" type="checkbox" checked>R8 混淆验证模块</label>
      <label><input id="vmp" type="checkbox" checked>运行时完整性、反调试和反 Hook 保护</label>
      <label><input id="fullVmp" type="checkbox" checked>整包 DEX 函数抽取兼容模式</label>
    </div>
    <div class="bar"><button id="start" disabled>开始处理 APK</button><button id="check" class="ghost" type="button">检测处理环境</button></div>
  </section>

  <section class="panel"><h2>处理状态</h2><div id="status" class="status">等待 APK...</div><div id="download"></div></section>
</main>
<script>
...
function choose(input){if(!input||!input.name.toLowerCase().endsWith('.apk')){log('请选择 APK 文件');return}selected=input;q('#fileName').textContent=input.name;start.disabled=false;log('已选择：'+input.name+'\\n点击“开始处理 APK”继续。')}
...
async function waitJob(id){for(let i=0;i<600;i++){await new Promise((resolve)=>setTimeout(resolve,2000));const response=await fetch('/api/jobs/'+encodeURIComponent(id),{cache:'no-store'});const job=await response.json();if(!response.ok)throw new Error(job.message||'读取任务失败');log((job.progress||'云端正在处理 APK')+'\\n\\n任务号：'+id);if(job.status==='done'&&job.result)return job.result;if(job.status==='failed')throw new Error(job.message||'APK 处理失败')}throw new Error('APK 处理超过 20 分钟')}
q('#check').onclick=async()=>{log('正在检测处理环境...');try{const response=await fetch('/api/status',{cache:'no-store'});const body=await response.json();if(!body.ok)throw new Error(body.message||'检测失败');const tools=body.tools||{};log('处理器正常\\nR8：'+(tools.r8?'可用':'不可用')+'\\n验证模块保护：'+(tools.vmp?'可用':'不可用')+'\\n整包函数抽取：'+(tools.fullVmp?'可用':'不可用'))}catch(error){log('检测失败：'+(error.message||error))}};
start.onclick=async()=>{if(!selected)return;start.disabled=true;download.innerHTML='';log('正在上传 APK...');const params=new URLSearchParams({fileName:selected.name,serverUrl:q('#server').value,appId:q('#appId').value,cardName:q('#cardName').value,purchaseUrl:q('#purchaseUrl').value,jumpText:q('#jumpText').value,jumpUrl:q('#jumpUrl').value,obfuscate:q('#obfuscate').checked?'1':'0',vmp:q('#vmp').checked?'1':'0',fullVmp:q('#fullVmp').checked?'1':'0'});try{const response=await fetch('/api/process?'+params.toString(),{method:'POST',body:selected});const first=await response.json();if(!response.ok||!first.ok)throw new Error(first.message||'提交失败');const body=first.queued?await waitJob(first.jobId):first;log('处理完成\\n包名：'+body.packageName+'\\n原启动页：'+body.launcher+'\\n'+(body.transportMessage||'V2 安全传输已启用')+'\\n'+(body.signingMessage||'')+'\\n'+body.obfuscationMessage+'\\n'+body.vmpMessage);const link=document.createElement('a');link.className='download';link.href=body.file;link.textContent='下载 '+body.fileName;download.appendChild(link)}catch(error){log('处理失败：\\n'+(error.message||error))}finally{start.disabled=false}};
</script></body></html>`;
}
